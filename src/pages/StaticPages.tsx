...
// 처리 방식(Prisma User/SoloRecord/DiaryRecord 모델, OAuth 흐름, 마이크 사용 방식 등)을
// 근거로 작성한 "초안"입니다. 법적 효력을 갖는 최종본이 아니므로, 게시 전 반드시 법무
// 검토를 거쳐 회사명/대표자/연락처 등 placeholder([ ] 표기)를 실제 정보로 교체하세요.
...
function DraftNotice({ kind }: { kind: 'terms' | 'privacy' }) {
  const label = kind === 'terms' ? '이용약관' : '개인정보처리방침';
  return (
    <p className="public-page-notice">
      <strong>[초안 — 법무 검토 전]</strong> 이 {label}은 서비스의 실제 데이터 처리 방식을
      근거로 작성된 초안이며, 시행 전 법무 검토를 거쳐 사업자 정보 등 세부 내용이 변경될 수
      있습니다. 최종 시행본은 이 페이지에 게시 시점과 함께 공지합니다.
    </p>
  );
}
...
export function TermsPage() {
  return (
    <PublicPage title="이용약관">
      <DraftNotice kind="terms" />
      <p className="muted">시행일: {LAST_UPDATED} (예정, 초안 기준)</p>

      <h2>제1조 (목적)</h2>
      <p>
        이 약관은 [회사명](이하 “회사”)가 제공하는 음성 크기 대결 서비스 “DeciDuel”(모바일
        애플리케이션 및 관련 제반 서비스, 이하 “서비스”)의 이용과 관련하여 회사와 회원 간의
        권리, 의무 및 책임사항, 이용조건 및 절차 등 기본적인 사항을 규정함을 목적으로 합니다.
      </p>

      <h2>제2조 (정의)</h2>
      <ul>
        <li>“서비스”란 회사가 제공하는 DeciDuel 앱 및 이를 통해 제공되는 모든 기능을 말합니다.</li>
        <li>
      ...
      <p>
        ...
        발생합니다. 회사는 관련 법령을 위배하지 않는 범위에서 이 약관을 개정할 수 있으며, 개정
      ...
      <ul>
        ...
        <li>
          ...
          하며, 변경하지 않아 발생하는 불이익에 대해 회사는 책임을 지지 않습니다.
        </li>
      </ul>

      <h2>제5조 (서비스의 제공 및 변경)</h2>
      <p>
        회사는 다음과 같은 서비스를 제공합니다: ① 실시간 음성 크기 대결(듀얼) 및 매칭, ② 솔로
        음량 측정 및 기록, ③ 다이어리(일별 기록) 작성·조회, ④ 글로벌 랭킹(리더보드), ⑤ 기타
        회사가 추가로 개발하거나 제휴를 통해 제공하는 서비스. 회사는 운영상·기술상 필요에 따라
      ...
      <ul>
        ...
        <li>
          서비스를 이용하여 얻은 정보를 회사의 사전 동의 없이 복제, 유통, 조장하거나 영리
          목적으로 이용하는 행위
        </li>
        <li>
        ...
        <li>회사 및 제3자의 지적재산권, 명예, 영업상 비밀 등을 침해하는 행위</li>
        <li>서비스의 안정적 운영을 방해할 수 있는 일체의 행위(부정 접속, 서버 공격 등)</li>
      </ul>

      <h2>제7조 (서비스 이용의 제한 및 정지)</h2>
      <p>
        회사는 회원이 제6조를 위반하거나 서비스의 정상적인 운영을 방해한 경우, 사전 통지 후(긴급
        시 사후 통지) 경고, 일시 정지, 영구 이용제한 등의 조치를 취할 수 있습니다. 회원은 회사의
      ...
      <p>
        ...
        지적재산권은 회사에 귀속됩니다. 회원이 서비스 내에서 직접 작성한 콘텐츠(닉네임, 다이어리
        코멘트 등)의 저작권은 해당 회원에게 있으며, 회원은 회사가 서비스의 운영·홍보·개선을
        위해 필요한 범위 내에서 이를 사용하도록 허락한 것으로 봅니다.
      </p>

      <h2>제10조 (면책조항)</h2>
      <p>
        회사는 천재지변, 불가항력적 사유, 회원의 귀책사유로 인한 서비스 장애, 회원이 서비스를
      ...
      <p>
        이 약관과 관련하여 분쟁이 발생한 경우 회사와 회원은 우선 상호 협의하여 해결하도록
      ...
      <h2>부칙</h2>
      <p>이 약관은 {LAST_UPDATED}부터 적용됩니다. (초안 — 시행일은 법무 검토 후 확정)</p>
    </PublicPage>
  );
}
...
export function PrivacyPage() {
  return (
    <PublicPage title="개인정보처리방침">
      <DraftNotice kind="privacy" />
      <p className="muted">시행일: {LAST_UPDATED} (예정, 초안 기준)</p>

      <p>
        [회사명](이하 “회사”)는 「개인정보 보호법」 등 관련 법령을 준수하며, 이용자의
        개인정보를 안전하게 처리하기 위해 다음과 같이 개인정보처리방침을 수립·공개합니다.
      </p>
      ...
      <ul>
        ...
        <li>
          ※ 회사는 Apple/Google/Kakao로부터 비밀번호 등 민감한 인증정보를 직접 전달받지
      ...
      <p>
        ...
        <strong>
          회사는 이용자의 음성을 녹음하거나 음성 파일·음성 데이터 자체를 서버로 전송·저장하지
          않습니다.
        </strong>{' '}
      ...
      <p>
        회사는 원칙적으로 회원 탈퇴 시 보유한 개인정보를 지체 없이 파기합니다. 다만 다음의
      ...
      <p>
        회사는 이용자의 개인정보를 제2조에서 명시한 목적 범위 내에서만 처리하며, 원칙적으로
      ...
      <h2>5. 개인정보 처리 위탁</h2>
      <p>
        회사는 원활한 서비스 제공을 위해 아래와 같이 개인정보 처리 업무를 외부 업체에 위탁하고
      ...
      <p>
        이용자는 언제든지 자신의 개인정보를 조회하거나 수정할 수 있으며, 회원 탈퇴(처리 정지
        요청)를 통해 동의를 철회할 수 있습니다. 닉네임·프로필 이미지·아바타 색상은 서비스 내
      ...
      <h2>9. 개인정보 보호책임자</h2>
      <p>
        회사는 개인정보 처리에 관한 업무를 총괄하고 이용자의 불만 처리 및 피해 구제를 위해
      ...
      <h2>부칙</h2>
      <p>이 방침은 {LAST_UPDATED}부터 적용됩니다. (초안 — 시행일은 법무 검토 후 확정)</p>
    </PublicPage>
  );
}
